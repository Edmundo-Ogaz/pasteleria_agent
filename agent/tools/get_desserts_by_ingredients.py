...
def get_desserts_by_ingredients(ingredients: list) -> str:
    # """function to get the products by a ingredient"""
    """Obtiene productos que contienen estos ingredientes."""
    print("*"*8,"get_desserts_by_ingredients", "*"*8)
    print(ingredients)

    query = """
    query SearchDessertsByIngredient($ingredients: [String!]!) {
        dessertsByIngredients(ingredientNames: $ingredients) {
            name
        }
    }
    """
    ...
    response = []
    if result and "data" in result and "dessertsByIngredients" in result["data"]:
        response = result["data"]["dessertsByIngredients"]
        
    if response:
        return f"los postres que tienen {formatear_lista(ingredients['ingredients'])} son: {response}"
    else:
